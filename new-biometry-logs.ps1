...
function Process-SingleDate {
    ...
    if ($results.results) {
        ...
        foreach ($result in $results.results) {
            ...
            # Detectar flujos mientras procesamos logs (pero no mostrar resultados aún)
            if ($logEntry['@message']) {
                # Buscar referencias a flujo vivere
                if ($logEntry['@message'] -match "(?i)flujo\s+de\s+vivere") {
                    $foundVivere = $true
                    $matchedVivereMessage = $logEntry['@message']
                }
                
                # Buscar referencias a flujo autogestivo
                if ($logEntry['@message'] -match "(?i)flujo\s+de\s+autogestivo") {
                    $foundAutogestivo = $true
                    $matchedAutogestivoMessage = $logEntry['@message']
                }
            }
        }
        
        # 2. SEGUNDO: Mostrar resultados del análisis de flujos DESPUÉS de todos los logs
